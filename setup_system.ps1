...
function Invoke-InPowerShellFile([string]$filePath, [string[]]$arguments = @()) {
...
if (-not $RunOnly) {
    ...
    Invoke-InVenv "pip install -r requirements.txt"
    ...
    # Write-Step "Preparing EduNavigator (Local Mistral Mode)"
    # Invoke-InPowerShellFile (Join-Path $root "start_edunavigator_local.ps1") @("-SetupOnly")
...
# # EduNavigator isolated backend - Local Mistral (port 8010)
# $eduNavigatorLauncher = Join-Path $root "start_edunavigator_local.ps1"
# Start-Process powershell -ArgumentList "-NoExit", "-ExecutionPolicy", "Bypass", "-File", "`"$eduNavigatorLauncher`"", "-Port", "8010"
...
# Write-Host "- EduNavigator: http://localhost:8010"
