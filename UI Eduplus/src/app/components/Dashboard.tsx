...
import { Brain, Bell, LogOut, MessageSquare, BarChart3, FileText, TrendingUp, Home, Target, Building2, Clock, ChevronRight, Zap, Bot, Compass } from "lucide-react";
import { Link } from "react-router";
import { motion } from "motion/react";
import { StudentSession } from "../auth";

const API_BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:5000/api";
const EDU_NAVIGATOR_URL = ((import.meta as any)?.env?.VITE_EDUNAVIGATOR_URL as string) || "http://localhost:8001";
...
export function Dashboard({ session, onLogout }: DashboardProps) {
  ...
  const navItems = [
    ...
    { icon: Compass, label: "EduNavigator", path: EDU_NAVIGATOR_URL, external: true },
          ...
          {navItems.map((item) => (
            item.external ? (
              <a key={item.label} href={item.path} target="_blank" rel="noreferrer">
                <button
                  className="w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors hover:bg-muted text-foreground"
                >
        ...
        <main className="flex-1 overflow-auto p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="p-6 border-primary/20">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl" style={{ fontWeight: 600 }}>EduNavigator</h3>
                  <p className="text-sm text-muted-foreground">Career roadmap and guided planning</p>
                </div>
              ...
              <p className="text-sm text-muted-foreground mb-4">
                Open EduNavigator in a separate isolated server for personalized guidance and mindmap-based planning.
              </p>
              <a href={EDU_NAVIGATOR_URL} target="_blank" rel="noreferrer">
