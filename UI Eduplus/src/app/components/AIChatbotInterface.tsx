...
import { Brain, Download, Home, Loader2, LogOut, Menu, MessageSquare, Mic, Paperclip, Send, Trash2, User, X } from "lucide-react";
...
const API_BASE_URL = ((import.meta as any)?.env?.VITE_LLM_API_URL as string) || "http://localhost:8001/api";
...
interface AIChatbotInterfaceProps {
  session: StudentSession;
  onLogout: () => void;
}

export function AIChatbotInterface({ session, onLogout }: AIChatbotInterfaceProps) {
  const welcomeText = "Hello! I am your EduPlus AI Chatbot (Local Mistral). Ask me about your predictions, companies, prep strategy, eligibility, and LeetCode company-wise interview questions.";
  ...
  const extractReportUrl = (text: string): string | null => {
    const match = text.match(/https?:\/\/[^\s]+\/api\/llm-reports\/[A-Za-z0-9._-]+\.pdf/i);
  ...
  const mapStoredMessages = (stored: StoredMessage[]): Message[] => {
    if (!stored || stored.length === 0) return getInitialMessages();
    return stored.map((m, idx) => {
      ...
      return {
        ...
        value: m.value,
        requiresPredictionUpdate: containsPredictionUpdateToken(rawText),
      };
    });
  };
  ...
  const suggestionChips = [
    "Show my placement prediction",
    "Recommend companies for my profile",
    "How to improve my DSA score?",
    "Top amazon leetcode questions in last 3 months",
    "Generate pdf for google leetcode interview questions",
    "Which companies allow lower cgpa?",
  ];

  const navItems = [
    { icon: Home, label: "Dashboard", path: "/dashboard" },
    { icon: MessageSquare, label: "Rasa Chatbot", path: "/chatbot" },
    { icon: Brain, label: "AI Chatbot", path: "/ai-chatbot", active: true },
  ];
  ...
  const loadHistory = async (sid: number, preferredChatId?: string | null) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/llm-chat-history/${sid}`);
      const data: ChatHistoryResponse = await response.json();
      if (!response.ok) throw new Error((data as any)?.error || "Failed to load chat history");
  ...
  const createNewChatForStudent = async (sid: number) => {
    try {
      const response = await fetch(`${API_BASE_URL}/llm-chat-history/${sid}/new`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: "New Chat" }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || "Unable to create new chat");
      ...
      if (newChatId) selectChat(newChatId, sessions);
      else {
        setMessages(getInitialMessages());
        setActiveChatId(null);
      }
  ...
  const handleDeleteChat = async (chatId: string) => {
    if (!studentId) return;
    try {
      const response = await fetch(`${API_BASE_URL}/llm-chat-history/${studentId}/${chatId}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok) throw new Error(data?.error || "Unable to delete chat");

      const history = data.history as ChatHistoryResponse;
      const sessions = history?.chats || [];
      setChatSessions(sessions);

      if (sessions.length === 0) await createNewChatForStudent(studentId);
      else {
        const next = history.active_chat_id || sessions[0].chat_id;
        selectChat(next, sessions);
      }
    } catch {
      // no-op
    }
  ...
  const handleClearHistory = async () => {
    if (!studentId) return;
    try {
      const response = await fetch(`${API_BASE_URL}/llm-chat-history/${studentId}`, { method: "DELETE" });
      await response.json();
      setChatSessions([]);
      setActiveChatId(null);
      setMessages(getInitialMessages());
  ...
  const handleSend = async () => {
    ...
    if (!activeChatId) await createNewChatForStudent(studentId);
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/llm-chatbot/message`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className="h-screen bg-background flex overflow-hidden">
      <aside
        className={`${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 fixed lg:static inset-y-0 left-0 z-40 w-64 bg-card border-r border-border transition-transform duration-300 flex flex-col`}
      >
        <div className="p-4 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 style={{ fontWeight: 600 }}>AI Chat History</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {historyLoading && <p className="text-xs text-muted-foreground">Loading chats...</p>}
          {!historyLoading && chatSessions.length === 0 && <p className="text-xs text-muted-foreground">No chat history yet.</p>}
          {!historyLoading &&
            chatSessions.map((chat) => (
              <div key={chat.chat_id} className="flex items-start gap-2">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 mt-1" onClick={() => handleDeleteChat(chat.chat_id)}>
        ...
        <div className="p-4 border-t border-border space-y-3">
          ...
          {navItems.map((item) => (
            <Link key={item.label} to={item.path}>
              <button className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${item.active ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}>
        ...
        <header className="bg-card border-b border-border px-4 lg:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 style={{ fontWeight: 600 }}>EduPlus AI Chatbot (Mistral)</h1>
                <div className="flex items-center gap-2">
                ...
                <div className={`flex gap-3 max-w-[85%] lg:max-w-[70%] ${message.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                      message.sender === "user" ? "bg-gradient-to-br from-[#003366] to-[#0055A4] text-white" : "bg-muted"
                    }`}
                  >
                    {message.sender === "user" ? <User className="w-5 h-5" /> : <Brain className="w-5 h-5 text-primary" />}
                  </div>

                  <div className="space-y-1">
                    <div className={`px-4 py-3 rounded-2xl ${message.sender === "user" ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
                      ...
                      {message.sender === "bot" && (() => {
                        ...
                        return (
                          <div className="mt-3">
                            <Button size="sm" className="h-8" onClick={() => window.open(reportUrl, "_blank")}>
                    ...
                    <p className={`text-xs text-muted-foreground px-2 ${message.sender === "user" ? "text-right" : "text-left"}`}>
                      {message.timestamp}
                    </p>
                  </div>
                </div>
          ...
          <div className="px-4 lg:px-6 pb-4">
            <div className="flex flex-wrap gap-2 justify-center">
              {suggestionChips.map((chip) => (
                <Button key={chip} variant="outline" size="sm" onClick={() => setInput(chip)} className="rounded-full">
                  {chip}
                </Button>
              ))}
            </div>
            ...
            <div className="flex items-end gap-2">
              ...
              <div className="flex-1 relative">
                <Input
                  ...
                  placeholder="Ask anything about placement, company criteria, predictions, prep strategy..."
              ...
              >
                {loadingResponse ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
              </Button>
            </div>
            <p className="text-xs text-muted-foreground text-center mt-2">Local Mistral + data-grounded responses. Verify critical placement policy with placement cell.</p>
          </div>
        </div>
      </div>
