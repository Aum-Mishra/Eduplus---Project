...
def get_student(student_id):
    ...
    try:
        if not os.path.exists(STUDENT_CSV):
            ...
            return jsonify({'exists': False, 'message': 'Student data file not found'}), 404
        ...
        # Ensure student_id column and input are the same type for comparison (string is safest)
        df['student_id'] = df['student_id'].astype(str)
        student = df[df['student_id'] == str(student_id)]
        
        print(f"[DEBUG] Found {len(student)} matching students for ID '{str(student_id)}'")
        ...
        if os.path.exists(PREDICTIONS_CSV):
            try:
                ...
                pred_df['student_id'] = pred_df['student_id'].astype(str)
                pred = pred_df[pred_df['student_id'] == str(student_id)]
                if not pred.empty:
                    last_prediction = pred.iloc[0].to_dict()
                    print(f"[DEBUG] Found existing prediction for student {student_id}")
                else:
                    print(f"[DEBUG] No prediction found for student {student_id} in {PREDICTIONS_CSV}")
            except Exception as pe:
                print(f"[DEBUG] No predictions file or error reading: {pe}")
        else:
            print(f"[DEBUG] Predictions file not found at {PREDICTIONS_CSV}")
