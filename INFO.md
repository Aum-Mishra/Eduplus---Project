...
The Windows workflow is split into isolated environment scripts:
- `setup_all.ps1` for first-time setup
- `start_all.ps1` to start the full system
- `stop_all.ps1` to stop the full system
- `check_services.ps1` to verify ports and health

Split Python dependencies are in:
- `requirements.backend.txt`
- `requirements.rasa.txt`
- `requirements.rag.txt`
- `requirements.common.txt` (legacy reference only)

## 8) Frontend and Python Dependency Model
- Python packages are split across three isolated requirements files to avoid dependency conflicts
